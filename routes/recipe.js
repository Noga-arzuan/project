...
module.exports = {
    ...
    if (uploadedFile.mimetype === 'image/png' || uploadedFile.mimetype === 'image/jpeg' || uploadedFile.mimetype === 'image/gif') {
        ...
        uploadedFile.mv(`public/assets/img/${image_name}`, (err) => {
            ...
            let query = "INSERT INTO `recipe_` ( name,kind,product_1,image)VALUES('" + name + "','" + kind + "','" + product_1 + "','" + image_name + "')";
...
editRecipePage: (req, res) => {
    ...
    editRecipe: (req, res) => {
        ...
        let query = "UPDATE `recipe_` SET `name` = '" + name + "', `kind` = '" + kind + "', `product_1` = '" + product_1 +  "' WHERE `recipe_`.`id` = '" + recipeId + "'";
        db.query(query, (err, result) => {
            if (err) {
                return res.status(500).send(err);
            }
